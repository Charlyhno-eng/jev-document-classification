...
export function GatewaySettings(props: Props) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) props.onClose(); }}>
    <section className="settings-modal" role="dialog" aria-modal="true" aria-labelledby="gateway-settings-title">
      ...
      {props.configured && <div className="gateway-balance"><div><span><WalletCards size={16} />Remaining credits</span><strong>{props.credits === null ? 'Temporarily unavailable' : `$${props.credits < 1 ? props.credits.toFixed(4) : props.credits.toFixed(2)}`}</strong></div><a className="gateway-link" href="https://vercel.com/ai-gateway" target="_blank" rel="noreferrer">Open Vercel <ExternalLink size={13} /></a></div>}
