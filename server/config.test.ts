...
test('supports an empty key but rejects malformed configuration', () => {
  assert.deepEqual(parseConfig('[classification]\ncategories = ["Legal"]\n\n[ai]\napi_key = ""\n'), { categories: ['Legal'], apiKey: '' });
  assert.throws(() => parseConfig('[classification]\ncategories = nope'));
  assert.throws(() => parseConfig('[classification]\ncategories = []'));
});

test('public configuration never contains the API key', () => {
  const publicConfig = toPublicConfig({ categories: ['Research'], apiKey: 'secret-key-' + 'x'.repeat(24) });
  assert.deepEqual(publicConfig, { categories: ['Research'], apiKeyConfigured: true });
...
test('creates a private config and preserves the key when categories change', async () => {
  ...
  assert.deepEqual(await store.read(), { categories: ['Whitepaper', 'Doctoral thesis'], apiKey: 'gateway-key-' + 'x'.repeat(24) });
...
test('serializes concurrent key and category updates without losing either value', async () => {
  ...
  assert.deepEqual(await store.read(), { categories: ['Research', 'Whitepaper'], apiKey: 'concurrent-key-' + 'x'.repeat(24) });
});
...
test('rejects a symlinked configuration file or directory', async () => {
  ...
  await writeFile(outside, serializeConfig({ categories: ['Research'], apiKey: '' }));
  ...
  await assert.rejects(() => new ConfigStore(path.join(linkedDirectory, 'config.toml')).write({ categories: ['Research'], apiKey: '' }), /symbolic link/);
});
